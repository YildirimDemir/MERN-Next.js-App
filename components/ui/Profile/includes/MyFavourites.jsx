...
export default function MyFavourites({ user, reqUserLikes }) {

    let updateReqUserLikes;
    if (user) {
        const { data: userLikes } = useQuery({
            queryKey: ["get-my-likes", user?.id],
            queryFn: async () => {
    ...
    return (
        <>
            {!updateReqUserLikes ? <DataLoader /> :
                <div className={`${Style["card-container"]}`}>
                    {updateReqUserLikes.length > 0 ? updateReqUserLikes?.map((tour) => (
